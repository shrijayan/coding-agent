...
[2026-08-14] Conversation summarization: trigger changed from MESSAGE-COUNT to TOKEN-BASED (real provider numbers, per the "never estimate" rule). AGENT_SUMMARY_THRESHOLD_MESSAGES removed; new AGENT_SUMMARY_TOKEN_THRESHOLD (default 8000) fires when the PREVIOUS send's real input_tokens (UsageTracker.last, new field set in record_llm_call) exceeds it. Message count no longer plays any part in the trigger. keep_recent_messages stays count-based (cut-point/coherence safety, safe_keep_from). Changes: metrics/usage.py (last field), optimizations/conversation_summary.py (token_threshold field, validation: threshold>0, keep_recent>=1), config.py + .env.example + local .env (env var swap), 3 test files' env fixtures updated, NEW tests/test_conversation_summary.py (9 tests: no-prior-call/below/above trigger, summarize recorded, incremental fold, validation - all passed). Notebook: harness cell default 8000, cell 13 narrative rewritten (token trigger stays quiet on small DEMO_PROMPTS - "short chats aren't its problem"; fires on real size not turn count), cell 15 demo now 6000/keep_recent=2 with token-based comment (big read pushes next send over threshold naturally). 155 tests pass. Slides NOT updated (no message-count claims on slides). NOT committed.

[2026-08-14] Conversation summarization (cont.): made the summarize event VISIBLE for the notebook's hands-on demo. Added ConversationSummaryEvent/ConversationSummaryTracker to optimizations/conversation_summary.py (folded message count, summary text, real summarize-call usage), wired via tracker param + get_tracker() + build() - same shape as ContextWindowTracker/ToolFilterTracker. Added 2 tracker tests (event recorded with real usage; no event below threshold) - 158 tests pass. Notebook: inserted a "Watch it happen - the summary fires live" markdown + code cell right after the first opt_summary compare (cells 13-14/68): threshold dropped to 6000 for the demo (save/restore), one prompt at a time via WorkshopSession(["conversation-summary"]), prints each turn's REAL last-send input tokens + session cost from UsageTracker.last/metrics, prints a SUMMARY FIRED line (folded count, actual summary text, summarize call's real in/out tokens) and the send-shrink comparison (before vs after fold) - no estimates anywhere. All notebook code cells AST-parse. NOT committed.
